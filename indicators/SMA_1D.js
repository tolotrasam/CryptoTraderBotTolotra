...
class SMA {
    constructor(array, obj, candle_size, graph_data, id) {
        ...
        this.daily_step_size = this.hours_step_size * 24;
        this.max_step_size_indicator = this.daily_step_size
    ...
    calculateIndicator(candle) {
        ...
        // if (n > 0) {
        //     this.cleaned_obj_current_market_data.min_step_change = (this.cleaned_obj_current_market_data.close_price - candle.close_price) / candle.close_price
        // }

        if (n >= this.daily_step_size) {
            this.cleaned_obj_current_market_data.daily_growth = (this.cleaned_obj_current_market_data.average_price - this.cleaned_array_offline_array_data[n - this.daily_step_size].average_price) / this.cleaned_array_offline_array_data[n - this.daily_step_size].average_price

            //calculating the moving average
            var sum = this.cleaned_obj_current_market_data.close_price;
            for (var i = 1; i < this.daily_step_size; i++) {
                sum = sum + this.cleaned_array_offline_array_data[n - i].average_price
            }
            // console.log(sum)
            this.cleaned_obj_current_market_data[this.id] = sum / this.daily_step_size;
        }

        //MA GROWTHS REQUIRES THE LENGH OF A SMA TO BE LOADED TWICE
        if (n >= this.daily_step_size * 2) {
            this.cleaned_obj_current_market_data.daily_ma24h_growth = (this.cleaned_obj_current_market_data[this.id] - this.cleaned_array_offline_array_data[n - this.daily_step_size][this.id]) / this.cleaned_array_offline_array_data[n - this.daily_step_size][this.id]
