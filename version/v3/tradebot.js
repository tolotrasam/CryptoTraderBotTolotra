...
//Real time trader
    ...
    online_data_mainlooper_bitfinex() {
        ...
        setTimeout(() => {
            if(this.continue){
                ...
                this.EmitStatus({state:'has stopped', msg: 'Bye :)'})
            }
    ...
    simulation_online() {
        ...
        this.to_date = this.now
        var data_fetch_from = this.getLastStepTimestamp() / 1000 - this.rangeInMinute * 60 * this.max_step_size_indicator
        this.dataminer.createSqlByTimeRangeAndMarket(this.exchange, this.asset, this.currency, this.candle_size, data_fetch_from, this.from_date, (raw) => {
    ...
    Simulation_offline() {
        ...
        this.dataminer.createSqlByTimeRangeAndMarket(this.exchange, this.asset, this.currency, this.candle_size, this.from_date, this.to_date, (raw) => {
            ...
            this.onInit({
                status: {state: 'Trading', msg: 'hello'},
                start_time: this.raw_array_data_db_format[0].start,
                start_price: this.raw_array_data_db_format[0].close,
                start_asset_balance: this.btc_balance,
                start_currency_balance: this.zar_balance,
            })
    ...
    stopBot(){
        this.continue = false;
        this.EmitStatus({state:'is Stoping soon...', msg: this.timeout_before_request})
    }
    getLastCandleUpdate() {
        var obj = {
    ...
    processNewCandle(rawArrayDataDbFormat) {
        ...
        if (this.requiresIndicatorsLoaded) {
